...
while ($true) {
    ...
    if ($Action -eq "1") {
        ...
        # --- SAFE MODE CHECK ---
        # FIX: anchor the match to the START of the answer ("^") instead of
        # searching anywhere in the string. The old pattern treated typing
        # "No" as a YES, because the lowercase "o" in "No" matched the
        # accepted-letters class (Y/O/J/S also covers "oui", "ja", "si").
        if ($SelectedDrives -contains "C") {
            $ConfirmC = Read-Host $UI.SafeWarn
            if ($ConfirmC.Trim() -notmatch "^[YOJSyojs]") {
        ...
        if ($TotalValue -gt 0) {
            try {
                ...
                # FIX: merge with whatever is already hidden instead of
                # overwriting NoDrives from scratch. Previously, hiding D
                # after already hiding C would silently un-hide C, because
                # each run replaced the whole bitmask rather than adding to it.
                $Existing = (Get-ItemProperty -Path $Path -Name "NoDrives" -ErrorAction SilentlyContinue).NoDrives
                if (-not $Existing) { $Existing = 0 }
                $CombinedValue = $Existing -bor $TotalValue

                Set-ItemProperty -Path $Path -Name "NoDrives" -Value $CombinedValue -Type DWord
                Write-Host $UI.Success -ForegroundColor Green
            } catch {
                Write-Host "Error applying changes: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
    }
    elseif ($Action -eq "2") {
    ...
    # FIX: same start-anchored match as above, for the same reason.
    $Restart = Read-Host $UI.Restart
    if ($Restart.Trim() -match "^[YOJSyojs]") {
        Stop-Process -Name explorer -Force -ErrorAction SilentlyContinue
        Start-Sleep -Milliseconds 500
        # FIX: explicitly relaunch Explorer instead of relying on Windows to
        # auto-restart it. Auto-restart-on-crash is the default behavior on
        # most systems, but it's not guaranteed on every configuration.
